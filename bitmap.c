 ...
 */
Bitmap *read_header() {
    // Read the BMP_HEADER_SIZE_OFFSET bytes to get the header size
    unsigned char headerSizeData[BMP_HEADER_SIZE_OFFSET];
    fread(headerSizeData, sizeof(unsigned char), BMP_HEADER_SIZE_OFFSET, stdin);

    // Extract the header size from the data
    int headerSize;
    memcpy(&headerSize, headerSizeData, BMP_HEADER_SIZE_OFFSET);

    // Allocate space for the header
    unsigned char *header = malloc(headerSize);

    // Copy the first BMP_HEADER_SIZE_OFFSET bytes to the beginning of the header
    memcpy(header, headerSizeData, BMP_HEADER_SIZE_OFFSET);

    // Read the remaining header data
    fread(header + BMP_HEADER_SIZE_OFFSET, sizeof(unsigned char), headerSize - BMP_HEADER_SIZE_OFFSET, stdin);

    // Extract width and height from the header
    int width = *(int *)(header + BMP_WIDTH_OFFSET);
    int height = *(int *)(header + BMP_HEIGHT_OFFSET);

    // Create and return a new Bitmap struct
...
void scale(Bitmap *bmp, int scale_factor) {
    ...
    // Update header size and allocate space for the new header
    int newHeaderSize = BMP_HEADER_SIZE_OFFSET + (bmp->width * 3 + 3) / 4 * 4 * bmp->height;  // Calculate new header size
    bmp->headerSize = newHeaderSize;

    // Allocate space for the new header
    unsigned char *newHeader = malloc(newHeaderSize);

    // Copy the original header data to the new header
    memcpy(newHeader, bmp->header, BMP_HEADER_SIZE_OFFSET);

    // Update width and height in the new header
    memcpy(newHeader + BMP_WIDTH_OFFSET, &(bmp->width), sizeof(int));
    memcpy(newHeader + BMP_HEIGHT_OFFSET, &(bmp->height), sizeof(int));

    // Free the old header
    free(bmp->header);

    // Update bmp to use the new header
    bmp->header = newHeader;
}
